...
export function DiagnosticoLocalSection({ ordemServico }: OrdemServicoDetalhesSectionProps) {
  ...
  return (
    <SectionCard title="Diagnóstico do local">
      ...
      <DetailItem label="Ponto de referência" value={[diagnosticoLocal.pontoReferencia]} />
      <div className="grid grid-cols-1 md:grid-cols-2 gap-md">
        <DetailItem label="Piscina" value={[formatYesNo(diagnosticoLocal.piscina)]} />
        <DetailItem label="Pet" value={[formatYesNo(diagnosticoLocal.pet)]} />
      </div>
    </SectionCard>
  );
}
