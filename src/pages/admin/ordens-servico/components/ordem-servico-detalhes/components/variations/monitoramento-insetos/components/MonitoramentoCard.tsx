import { ChevronDown } from "lucide-react";
import { useState } from "react";
import { BeakerFilledIcon } from "@/assets/icons/beaker-filled";
import { BugAntIcon } from "@/assets/icons/bug-ant";
import { DetailItem } from "@/atomic/atm.detail-item";
import { Body1, Body2, H3, H4 } from "@/atomic/atm.typography";
import { ImageCarousel } from "@/atomic/mol.image-carousel";
import { cn } from "@/lib/utils";
import type { OrdemServico } from "@/model/rest/ordem-servico";
import { formatYesNo } from "@/utils/formatters";

interface MonitoramentoCardProps {
  monitoramento: OrdemServico["monitoramento"][number];
}

export function MonitoramentoCard({ monitoramento }: MonitoramentoCardProps) {
  const [isOpen, setIsOpen] = useState(false);

  return (
    <div className="flex flex-col gap-xs p-md border border-grayscale-light rounded-2xl">
      <button type="button" className="w-full cursor-pointer" onClick={() => setIsOpen(!isOpen)}>
        <div className="flex flex-col gap-xs">
          <div className="flex items-center justify-between">
            <H3 className="font-normal">{monitoramento?.areaMonitorada}</H3>
            <ChevronDown
              className={cn(
                "size-[26px] text-grayscale-dark transition-transform",
                isOpen && "rotate-180",
              )}
            />
          </div>
          <div className="flex items-center gap-md">
            <div className="flex items-center gap-2xs">
              <BugAntIcon className="size-lg text-grayscale-dark" />
              <Body2 className="text-grayscale-dark">{monitoramento?.pragaAlvo}</Body2>
            </div>
            <div className="flex items-center gap-2xs">
              <BugAntIcon className="size-lg text-grayscale-dark" />
              <Body2 className="text-grayscale-dark">{monitoramento?.grauInfestacao}</Body2>
            </div>
            <div className="flex items-center gap-2xs">
              <BeakerFilledIcon className="size-lg text-grayscale-dark" />
              <Body2 className="text-grayscale-dark">{monitoramento?.tratamento}</Body2>
            </div>
          </div>
        </div>
      </button>

      {isOpen && (
        <>
          <div className="w-full h-px bg-grayscale-light" />
          <div className="flex gap-xs">
            <DetailItem label="Grau de infestação" value={[monitoramento?.grauInfestacao]} />
            <DetailItem label="Produto utilizado" value={[monitoramento?.produtoUtilizado]} />
          </div>
          <DetailItem label="Adesiva" value={[monitoramento?.adesiva]} />

          <div className="w-full h-px bg-grayscale-light" />

          <div className="flex gap-xs">
            <DetailItem label="Produto" value={[monitoramento?.produto]} />
            <DetailItem label="ml/L ou g/m³" value={[monitoramento?.ml]} />
          </div>
          <div className="flex gap-xs">
            <DetailItem
              label="Refil luminosa"
              value={[formatYesNo(monitoramento?.refilLuminosa)]}
            />
            <DetailItem label="Quantidade" value={[monitoramento?.quantidade]} />
          </div>

          <div className="w-full h-px bg-grayscale-light" />
          <H3 className="font-normal text-grayscale-dark">Fotos do serviço</H3>
          <ImageCarousel images={monitoramento?.fotos} pageSize={4} />
          <H4>Observações gerais</H4>
          <Body1 className="font-normal text-grayscale-dark">
            {monitoramento?.observacoes ?? "-"}
          </Body1>
        </>
      )}
    </div>
  );
}
