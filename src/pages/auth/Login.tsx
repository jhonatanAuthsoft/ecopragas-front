...
export default function Login() {
  ...
  const isTechnician = location.pathname === ROUTES.AUTH.LOGIN.TECHNICIAN;
  ...
  return (
    <div className='flex min-h-screen w-full items-center justify-center bg-grayscale-x-light p-6'>
      <div className='flex w-full overflow-hidden rounded-2xl justify-center gap-lg lg:grid-cols-[1fr_1.15fr]'>
          ...
          <div className='w-full'>
            ...
            <form onSubmit={handleLogin} className='mt-10 space-y-5'>
              <div className='space-y-2'>
                <Label
                  htmlFor='email'
                  className='text-xxs font-medium text-grayscale-black'
                >
                  {isTechnician ? 'CPF' : 'Email'}
                </Label>
                <Input
                  id='email'
                  type={isTechnician ? 'text' : 'email'}
                  placeholder={
                    isTechnician ? '000.000.000-00' : 'Digite seu email'
                  }
                  value={loginEmail}
                  onChange={(e) => {
                    const value = e.target.value;
                    setLoginEmail(isTechnician ? formatCPFCNPJ(value) : value);
                  }}
                  required
        ...
        <img
          src={
            isTechnician ? '/presentation-tech.png' : '/presentation-frame.png'
          }
          alt=''
