  ...
  return (
    <div className={cn('relative inline-block text-left w-full md:w-auto', className)} ref={dropdownRef}>
      <div>
        ...
        >
          <div className='flex items-center gap-xs'>
            <CalendarIcon
              className='text-grayscale-medium size-md'
            />
            <Body2 className='text-grayscale-dark font-medium'>
              {getSelectedText()}
            </Body2>
          </div>
          <ChevronDown 
      ...
      {isOpen && (
        <>
          {/* Overlay for mobile */}
          <div className='md:hidden fixed inset-0 bg-grayscale-x-dark/20 z-40' onClick={() => setIsOpen(false)} />
