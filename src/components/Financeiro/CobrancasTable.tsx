...
import { MoreHorizontal, Download, Check, X, Eye } from "lucide-react";
...
export const CobrancasTable = ({ cobrancas, onRefresh }: CobrancasTableProps) => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "pendente":
        return <Badge variant="outline" className="bg-yellow-500/10 text-yellow-700">Pendente</Badge>;
      case "pago":
        return <Badge variant="outline" className="bg-green-500/10 text-green-700">Pago</Badge>;
      case "vencido":
        return <Badge variant="outline" className="bg-red-500/10 text-red-700">Vencido</Badge>;
      case "cancelado":
        return <Badge variant="outline" className="bg-gray-500/10 text-gray-700">Cancelado</Badge>;
      default:
          ...
          {cobrancas.length === 0 ? (
            ...
            cobrancas.map((cobranca) => (
              <TableRow key={cobranca.id}>
                ...
                <TableCell>
                  {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(Number(cobranca.valor))}
                </TableCell>
                <TableCell>
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      ...
                      {cobranca.tipo_pagamento === "pix" && (
                        <DropdownMenuItem onClick={() => copiarPixCopiaCola(cobranca.pix_copia_cola)}>
